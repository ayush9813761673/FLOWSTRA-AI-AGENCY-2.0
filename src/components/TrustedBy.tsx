...
export function TrustedBy() {
  const logos = [
    "Zapier", "Make.com", "OpenAI", "Anthropic", "HubSpot", "Airtable", "Google",
    "Zapier", "Make.com", "OpenAI", "Anthropic", "HubSpot", "Airtable", "Google",
    "Zapier", "Make.com", "OpenAI", "Anthropic", "HubSpot", "Airtable", "Google",
    "Zapier", "Make.com", "OpenAI", "Anthropic", "HubSpot", "Airtable", "Google"
  ];
    ...
    <section className="w-full border-b border-white/5 bg-black/30 py-10 overflow-hidden relative flex flex-col items-center">
      ...
      <div className="relative flex w-full overflow-hidden">
        ...
        <div className="marquee-container">
          <div className="flex">
            {logos.map((logo, idx) => (
              <div key={`logo-set-1-${idx}`} className="flex justify-center w-[200px] grayscale opacity-40 hover:opacity-100 transition-opacity duration-300">
          ...
          <div className="flex">
            {logos.map((logo, idx) => (
              <div key={`logo-set-2-${idx}`} className="flex justify-center w-[200px] grayscale opacity-40 hover:opacity-100 transition-opacity duration-300">
                <span className="text-xl font-bold tracking-tight text-white/70">{logo}</span>
              </div>
            ))}
          </div>
